...
func NextState(state *State, actions []int, env *env.Env, rnd *rand.Rand) (*State, *pos.Pos, []float64) {
	agentItems, posItems, lastAppear, rewards := nextItems(state, actions, env, rnd)
	agentPos := nextPos(state, actions, env, rnd)
	return &State{Turn: state.Turn + 1, AgentItems: agentItems, AgentPos: agentPos, PosItems: posItems, RandomValues: state.RandomValues}, lastAppear, rewards
}
